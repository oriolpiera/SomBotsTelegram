...
reply_keyboard = [['Número de dies de la factura', 'Potència contractada'],
...
def done(bot, update, user_data):
    ...
    dies = float(user_data['Número de dies de la factura'])
    lloguerComptador = float(user_data['Lloguer comptador'])
    consum =  (float(user_data['Consum en kWh'])*0.130)
    print("Consum: %f" % consum)
    potencia =  round((float(user_data['Potència contractada']) * 38.043426 * dies / 365),2)
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            CHOOSING: [RegexHandler('^(Número de dies de la factura|Potència contractada|Consum en kWh|Lloguer comptador)$',
                                    regular_choice,
                                    pass_user_data=True),
                       ],
